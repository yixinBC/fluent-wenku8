...
def _login(username: str, password: str, usecookie: int) -> RequestsCookieJar:
    ...
    return requests.post(
        "https://www.wenku8.net/login.php", form_data, headers=FAKE_HEADERS
    ).cookies
...
class Book:
    ...
    def __init__(self, cookies: RequestsCookieJar, book_id: str) -> None:
        ...
        self.meta_info = BeautifulSoup(
            requests.get(
                self.book_url, cookies=self.cookies, headers=FAKE_HEADERS
            ).content,
            "lxml",
        )
        self.name = self.meta_info.select(
            "#content > div:nth-child(1) > table:nth-child(1) > tr:nth-child(1) > "
            "td:nth-child(1) > table:nth-child(1) > tr:nth-child(1) > td:nth-child("
            "1) > span:nth-child(1) > b:nth-child(1)"
        )[0].text
        self.cover_url = self.meta_info.select(
            "#content > div:nth-child(1) > table:nth-child(4) > "
            "tr:nth-child(1) > td:nth-child(1) > img:nth-child(1)"
        )[0]["src"]
        self.author = Author(
            self.cookies,
            self.meta_info.select(
